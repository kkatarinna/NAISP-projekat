...
func (mr MerkleRoot) FormMerkleTree(data [][]byte,serialize bool) MerkleRoot{
	...
	for _,element:= range data{
		d := Hash(element)
		leaf := Node{data:d[:],left:nil,right:nil}
	...
	for{
		h2 := make([]Node,0)
		if(len(h1) %2 != 0){
			h1 = append(h1,Node{data:[]byte{'0'},left:nil,right: nil})
		}
		for{
			if(i > len(h1) -1){
				break
			...
			d := Hash(combine)
			node := Node{data:d[:], left:&left, right:&right}
			h2 = append(h2,node)
			i +=2
		}
		i = 0
...
func PrintMerkleInOrder(myNode *Node){
	if(myNode != nil){
...
func PrintMerkleWidth(myNode *Node){
	h := make([]*Node,0)
	h = append(h,myNode)
	h2 := make([]*Node,0)
	i := 0
